export class CloudAtlas {
    ...
    on(method, path) {        
        const rndx = this.routes.push({path: (path=(group=(this.current.group.path || this.group(''))) + path), method, group: group, mw: [], error: [], pattern: new RegExp(`^${path.replace(/:(\w+)/g, (_, paramName) => `(?<${paramName}>[^/]+)`)}$`)});
    ...
    async handle(con) {
        ...
        catch (err) {(handler = [...route.error, ...route.group.error].find(h => err instanceof h.error)) ? await handler.method(err, con) : (() => { throw err; })()} 
        finally {await this.compose(route.group.amw)(con)}
        return con.response();
    }

    compose(middlewares) {return async (ctx) => { await (dispatch = async (index) => { index < middlewares.length ? await middlewares[index](ctx, () => dispatch(index + 1)) : Promise.resolve() })(0)}}
